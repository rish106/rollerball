...
int eval(Board& b) {
    ...
    for (int i = 0; i < 6; i++) {
        if (player_pieces[i] != DEAD) {
            ...
            score += weights[i];
            // score += weights[i] + (weights[i]/(20+abs(ky-tempy)+(abs(kx-tempx))));
            // if (i >= 4){
            //     int px, py;
...
int minimax(Board& board, int depth, bool maximizing_player, vector<Board*> &visited, int &alpha, int &beta) {
    if (maximizing_player && mp[all_boards_to_str(board)] == 2) {
        return INT_MAX;
    } else if (!maximizing_player && mp[all_boards_to_str(board)] == 2) {
        return INT_MIN;
    }
    if (depth == 0) {
        return eval(board);
